...
{
    std::string request = "INSERT INTO Customers (uuid, firstName, name, email, phone, address, gender, passportId) VALUES ('" + 
                          customer.m_uuid + "', '" + 
                          customer.m_firstName + "', '" + 
                          customer.m_name + "', '" + 
                          customer.m_email + "', '" + 
                          customer.m_phone + "', '" + 
                          customer.m_address + "', '" +
                          customer.m_gender + "', '" +
                          customer.m_passportId + "')";
    try {
        m_db.exec(request);
...
{
    std::string request = "UPDATE Customers SET firstName = '" + customer.m_firstName + 
                          "', name = '" + customer.m_name + 
                          "', email = '" + customer.m_email + 
                          "', phone = '" + customer.m_phone + 
                          "', address = '" + customer.m_address +
                          "', gender = '" + customer.m_gender + 
                          "', passportId = '" + customer.m_passportId + 
...
{
    std::string request = "SELECT id, uuid, firstName, name, email, phone, address, gender, passportId FROM Customers WHERE uuid = '" + customer_id + "'";
...
{
    std::string request = "SELECT id, uuid, firstName, name, email, phone, address, gender, passportId FROM Customers";
    SQLite::Statement query(m_db, request);
...
void CustomerDataAccess::init_database()
{
    if (create_database_directory())
    {
        ...
        {
            m_db.exec("CREATE TABLE Customers (id INTEGER PRIMARY KEY, uuid TEXT, firstName TEXT, name TEXT, email TEXT, phone TEXT, address TEXT, gender TEXT, passportId TEXT)");
